...
//        students.stream()
//                .filter(s -> "Argentina".equalsIgnoreCase(s.getCountry()))
//                .forEach(System.out::println);
        // 2. Print students whose last names starts with 'T'.
//        students.stream()
//                .filter(s -> s.getLastName().startsWith("T"))
//                .forEach(System.out::println);
...
//                .forEach(System.out::println);
        ...
        boolean fromMaldives = students.stream()
                .anyMatch(s -> "Maldives".equalsIgnoreCase(s.getCountry()));
        System.out.println(fromMaldives);
        // 7. Does everyone have a non-null, non-empty email address?
        boolean hasEmail = students.stream()
                .allMatch(s -> s.getEmailAddress() != null && !s.getEmailAddress().isEmpty());
...
//                                .anyMatch(r -> "Literary Genres".equalsIgnoreCase(r.getCourse())))
//                                .forEach(System.out::println);
        // 10. Who has the latest birthday? Who is the youngest?
//        Optional<Student> youngest = students.stream()
//                .max(Comparator.comparing(Student::getBirthDate).reversed());
...
//        studentsPerCountrySorted.forEach((country, count) -> System.out.println(country + count));
...
//        registrationsPerCourse.forEach((course, count) -> System.out.println(course+count));
        ...
        Map<String, Double> gpaPerCountry = students.stream()
                .collect(Collectors.groupingBy(Student::getCountry,
                        Collectors.averagingDouble(s -> s.getGpa().doubleValue())));
        gpaPerCountry.forEach((country, avgGpa) -> System.out.println(country + " " + avgGpa));
